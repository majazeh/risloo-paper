...
ap.add_argument("-i", "--image_files", required=True,
	help="path to the input image files" ,nargs='*')
args = vars(ap.parse_args())

if args["image_files"] is None:
    raise NameError("You didn't enter any arguement !")
...
for f in image_files :
    if not os.path.exists(f):
        raise NameError("This file : " , f ,"was NOT found!")
    if not os.path.isfile(f) :
        raise NameError("This is not a file : " , f )        
    if not f.endswith( ('jpg','png')) :
        raise NameError("This is not an image file : " , f )        
...
############################################### STEP 5 :Template matching for find our page #############################################
    ...
    cv2.imwrite(current_directory + r"/adjusted_tests/adjusted_" + tail , paper )
...
############################################################## ############################################################## 
    ...
    while( i < len(questionCnts)):
        ...
        for ncol in range(ncols):
            ...
            if counter >=1 :
                ...
                temp_Answers[str(num_questions_last_pages + num_question_each_col * ncol +(q+1))] = choices
                location_Answers[str(num_questions_last_pages + num_question_each_col * ncol +(q+1))] = locations
    ...
    num_questions_last_pages = num_questions +  num_questions_last_pages
    print("num_questions_last_pages" ,num_questions_last_pages)  

    Answers['qrcode'] = barcodeData
    Answers['papers'].append(current_directory + r"/adjusted_tests/adjusted_" + tail)
    ###################################################################################################
...
save_path = current_directory+ r"/Answers/Answers_" + tail[:-4] + r".json"
